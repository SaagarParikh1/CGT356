...
<body>
<h1 style="font-size:14pt; text-indent:360px;">Assign 04 - Index Page</h1>
...
<form id="form0" method="post" action="storeinfo.php"> 
    <fieldset id="billing">
        ...
        <ul>
            ...
            <li> <label title="Country" for="country">Country <span>*</span></label> <input type="text" name="country" id="country" size="30" maxlength="30" value="<?php if( !empty($_SESSION["country"])){echo($_SESSION["country"]); }?>"/></li>
            ...
            <li> <label title="Comments" for="comments">Questions or Comments <span>*</span></label> 
				<textarea rows="5" cols="40" name="comments" id="comments"><?php if( !empty($_SESSION["comments"])){echo($_SESSION["comments"]); } ?></textarea></li>
        </ul>
    </fieldset>
    <fieldset id="shipping">
        <legend>Shipping Information (if different from billing)</legend>
        <ul>
            <li> <label title="SName" for="Sname">Name </label> <input type="text" name="Sname" id="Sname" size="30" maxlength="30" value="<?php if( !empty($_SESSION["Sname"])){echo($_SESSION["Sname"]); }?>"/></li>
            <li> <label title="SAddress" for="Saddress">Address </label> <input type="text" name="Saddress" id="Saddress" size="30" maxlength="30" value="<?php if( !empty($_SESSION["Saddress"])){echo($_SESSION["Saddress"]); }?>"/></li>
            <li> <label title="SCity" for="Scity">City </label> <input type="text" name="Scity" id="Scity" size="30" maxlength="30" value="<?php if( !empty($_SESSION["Scity"])){echo($_SESSION["Scity"]); }?>"/></li>
            <li> <label title="SState" for="Sstate">State </label> <input type="text" name="Sstate" id="Sstate" size="30" maxlength="30"  value="<?php if( !empty($_SESSION["Sstate"])){echo($_SESSION["Sstate"]); }?>" /></li>
            <li> <label title="SZip" for="Szip">Zip Code </label> <input type="text" name="Szip" id="Szip" size="7" maxlength="5" value="<?php if( !empty($_SESSION["Szip"])){echo($_SESSION["Szip"]); }?>" /></li>
